...
import { mapSubjectRow, subjectPayload, subjectsRepository } from '../js/services/subjectsRepository.js';
import { mapVocabularyRow, vocabularyPayload, vocabularyRepository } from '../js/services/vocabularyRepository.js';
...
test('subject repository saves a collection in one upsert request', async () => {
    const calls = [];
    const client = {
        from(table) {
            assert.equal(table, 'subjects');
            return {
                upsert(payloads, options) {
                    calls.push({ payloads, options });
    ...
    const saved = await withFakeClient(client, () => subjectsRepository.saveAll([
        { slug: 'technology', name: 'Technology', sortOrder: 10, active: true },
        { slug: 'science', name: 'Science', sortOrder: 20, active: true }
    ]));

    assert.equal(calls.length, 1);
    assert.equal(calls[0].payloads.length, 2);
    assert.deepEqual(saved.map(subject => subject.slug), ['technology', 'science']);
});
...
test('vocabulary placement updates use one bounded RPC payload', async () => {
    const calls = [];
    const client = {
        rpc(name, args) {
            calls.push({ name, args });
            return Promise.resolve({ data: { requested: 2, updated: 2 }, error: null });
        }
    };

    const result = await withFakeClient(client, () => vocabularyRepository.updateVocabularyPlacements([
        { id: 'one', assignedDate: '2026-08-10', trimester: 'IIT', month: 'August', week: 2 },
        { id: 'two', assignedDate: '2026-08-17', trimester: 'IIT', month: 'August', week: 3 }
    ]));

    assert.equal(calls.length, 1);
    assert.equal(calls[0].name, 'update_vocabulary_placements_v1');
    assert.equal(calls[0].args.p_updates.length, 2);
    assert.deepEqual(result, { requested: 2, updated: 2 });
});
