...
test('Illustration runtime responsibilities have one complete owner each', () => {
    ...
    assert.equal(methodNames.length, 58);
...
test('Word Hunt research keeps definitions, examples, and images short and topic-aware', () => {
    const activity = createActivity();
    activity.researchContext = {
        grade: '6',
        subjectName: 'Technology',
        subjectSlug: 'technology',
        unitName: 'Grade 6 T2 Practice: June Week 2 - Movement Instructions'
    };

    const links = activity.getResearchLinks({
        word: 'speed',
        definition: 'How quickly the robot moves during a simple route.'
    });
    const definitionPrompt = decodeURIComponent(new URL(links[0][1]).searchParams.get('q'));
    const imageQuery = decodeURIComponent(new URL(links[1][1]).searchParams.get('q'));
    const examplesPrompt = decodeURIComponent(new URL(links[2][1]).searchParams.get('q'));

    assert.equal(activity.getResearchTopic(), 'Movement Instructions');
    assert.match(definitionPrompt, /Topic: Movement Instructions/);
    assert.match(definitionPrompt, /using 5 to 12 words/);
    assert.match(examplesPrompt, /Each sentence must use 6 to 12 words/);
    assert.equal(
        imageQuery,
        'speed in the context of Movement Instructions: How quickly the robot moves during a simple route.'
    );
});
