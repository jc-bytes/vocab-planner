/* ============================================================
   Trapdoor Trials - a surprise-trap platformer.
...
const PALETTES = {
  dark: {
    ...
    reset: "#64d8ff", resetDark: "#5b8cff", dust: "#565b69",
  ...
  light: {
    ...
    reset: "#168fbd", resetDark: "#315fc4", dust: "#c0b8a8",
...
function applyTheme(mode, save = true) {
  ...
  if (save) { try { localStorage.setItem("tt_theme", mode); } catch {} }
...
const AudioFX = (() => {
  ...
  return {
    ...
    trick: () => { [300, 260, 300, 260, 220].forEach((f, i) => setTimeout(() => tone(f, 0.09, "sawtooth", 0.06), i * 110)); },
...
// ---------------------------------------------------------------- particles
const particles = [];
function spawnResetBurst(x, y) {
  for (let i = 0; i < 30; i++) {
    const a = rand(0, Math.PI * 2), s = rand(90, 320);
    particles.push({
      x, y, vx: Math.cos(a) * s, vy: Math.sin(a) * s,
      r: rand(2.5, 6), life: rand(0.35, 0.85), t: 0,
      color: Math.random() < 0.7 ? theme.reset : theme.resetDark, grav: false,
    });
  }
}
  ...
  update(dt, g) {
    ...
    if (this.i < this.positions.length - 1) {
      ...
      if (Math.hypot(dx, dy) < this.fleeDist) {
        ...
        spawnPoof(this.pos.x + this.w / 2, this.pos.y + this.h / 2);
        AudioFX.poof();
        if (this.i === this.positions.length - 1) AudioFX.trick();
      }
    }
  }
  playerWins(p) {
  ...
  // ---------------------------------------------------- 30 (NEW: finale)
  {
    ...
    build: () => ({
      ...
      traps: [
        ...
        new Note(150, 250, "one last surprise"),
      ],
    }),
  },
];
...
const DEATH_LINES = [
  "RESET!", "NEW CLUE UNLOCKED.", "SO CLOSE.", "TRY A NEW ROUTE.", "YOU FOUND THE TRAP.",
  "ADAPT AND ADVANCE.", "WATCH THE FLOOR.", "TIMING IS THE KEY.", "ONE MORE TRY.",
  "GOOD DISCOVERY.", "PAUSE, PLAN, GO.", "YOU'VE GOT THIS.",
];
const ROASTS = [
  [0, "A flawless run!"],
  [25, "Excellent observation and timing."],
  [75, "Strong persistence through every surprise."],
  [150, "You kept learning until the course was clear."],
  [9999, "Every attempt helped solve the course."],
];
...
// ---------------------------------------------------------------- progress / DOM
function getDone() {
  try { return JSON.parse(localStorage.getItem("tt_done")) || {}; } catch { return {}; }
}
function saveProgress() { localStorage.setItem("tt_deaths", Game.deaths); }
function loadProgress() {
  const d = localStorage.getItem("tt_deaths");
  Game.deaths = parseInt(d) || 0;
}
function getProgressSnapshot() {
  const done = getDone();
  const completedLevels = LEVELS.reduce((count, _level, index) => count + (done[index] ? 1 : 0), 0);
  const completed = completedLevels === LEVELS.length;
  const attemptBonus = Math.max(0, 9999 - Math.min(Game.runDeaths, 9999));
  return {
    score: completedLevels * 100000 + attemptBonus + (completed ? 100000 : 0),
    level: Math.min(LEVELS.length, Game.levelIndex + 1),
    deaths: Game.runDeaths,
...
function reportProgress(gameOver) {
  window.parent.postMessage({
    type: "trapdoor-trials-score",
    ...getProgressSnapshot(),
    gameOver,
  }, "*");
}
function updateDeathHud() {
  document.getElementById("hud-deaths").textContent = Game.runDeaths;
...
function showEnd() {
  ...
  Game.state = "end";
  document.getElementById("end-deaths").textContent = Game.runDeaths;
...
(function initTheme() {
  let saved = null;
  try { saved = localStorage.getItem("tt_theme"); } catch {}
