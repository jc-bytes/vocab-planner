...
| 21. Remove duplicated navigation wiring | IN PROGRESS | Teacher history listener; registry-derived reverse view mapping; navigation generation/owner tokens; primary route reservations; lazy-feature race tests | Focused teacher feature suites and browser races, all-seven-page smoke, complete regression suite, production build, independent reviews | History has one authority and stale lazy page work is invalidated before context creation. Deep asynchronous editor work and direct navigation bypasses remain the final bounded lifecycle work. |
