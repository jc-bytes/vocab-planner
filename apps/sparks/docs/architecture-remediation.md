...
| 29. Add interfaces only where justified | DONE | Signed-in reward authority; narrow teacher auth capability; fail-closed role lifecycle; game/server score parity | Focused/full regression; production builds; registry/database contracts; independent reviews | Added one justified injectable seam for teacher auth; signed-in rewards now consume the existing authoritative server result without another wrapper. Client leaderboard capability exactly matches independent SQL authorization and score order. |
...
- Verification: focused auth/API tests, the same-profile authenticated smoke, complete source suite on an isolated port, local Supabase acceptance, dependency audit, production build and delivery budgets, built-site smoke, and seven-width responsive audit passed locally. Database lint reported only its two pre-existing extra warnings; database advisors reported no issues. GitHub Pages workflow [32877332386](https://github.com/jc-bytes/vocab-planner/actions/runs/32877332386) then passed the complete validation gate, including authenticated student/teacher smoke in one browser context, Firefox and WebKit focused smoke, responsive audit, authorization acceptance, production build, built-site smoke, and deployment.
