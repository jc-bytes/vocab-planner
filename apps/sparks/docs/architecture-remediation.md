...
| 20. Migrate teacher pages | IN PROGRESS | Overview, Vocabulary, Sparks, and Students shell/route migrations; primary-page browser smoke; Students/Groups account isolation; Data roster-control cleanup | Per-page registry, routing, navigation, browser history, feature, account-switch, and UI checks | Four primary pages are migrated. Student Progress and Groups account-scoped state are hardened; Data roster controls are scrubbed on disposal. Delegated Settings continuations are the remaining account-boundary check before page migration continues. |
