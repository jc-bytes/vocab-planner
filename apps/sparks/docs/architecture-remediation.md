...
| 12. Clean up owned inline styles | IN PROGRESS | Quiz Maker, Analytics Dashboard, Student Progress roster, Add Student, Student Detail, loading/login, and Vocabulary presentation; inline-style/specificity contracts | Complete suites, focused ownership/lazy checks, multi-viewport computed/pixel comparisons, production builds, source/built smoke | Seven bounded teacher surfaces now delegate static presentation to owned CSS. Runtime state and user-selected colors remain with their behavior/data owners; data-management surfaces remain to inspect incrementally. |
