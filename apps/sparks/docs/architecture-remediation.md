...
| 12. Clean up owned inline styles | IN PROGRESS | Quiz Maker, Analytics Dashboard, Student Progress roster, Add Student, Student Detail, loading/login, Vocabulary, Settings, Data Export, and Data Viewer presentation; inline-style/specificity contracts | Complete suites, focused ownership/lazy checks, multi-state and multi-viewport computed/pixel comparisons, production builds, source/built smoke | Ten bounded teacher surfaces now delegate static presentation to owned CSS. Runtime visibility, drag state, calculated progress, reset locking, and user-selected colors remain with their behavior/data owners; Data Reset remains to inspect separately. |
