...
| 21. Remove duplicated navigation wiring | IN PROGRESS | Teacher history listener; registry-derived reverse view mapping; navigation generation/owner tokens; primary route reservations; lazy-feature and editor restoration race tests | Focused teacher feature/editor suites and browser races, all-seven-page smoke, complete regression suites, production builds, independent reviews | History has one authority; stale lazy and Vocabulary-opening work is invalidated. Vocabulary account/session and queued-save ownership are the final Task 21 reliability work. |
...
Task 21 remains in progress. Independent tracing found a separate account-isolation seam: retained Vocabulary/cache state and queued saves can cross a teacher-session change even when page navigation is current. That will be fixed as its own security-focused task before Task 21 is complete; the device-global local draft policy remains a separate product decision and will not be silently changed.
