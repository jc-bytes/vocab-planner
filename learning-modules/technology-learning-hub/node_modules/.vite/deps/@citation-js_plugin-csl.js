  ...
  "node_modules/citeproc/citeproc_commonjs.js"(exports, module) {
    ...
    CSL2.Output.Formats.prototype.html = {
      ...
      "text_escape": function(text) {
        ...
        return text.replace(/&/g, "&#38;").replace(/</g, "&#60;").replace(/>/g, "&#62;").replace(/\s\s/g, "  ").replace(
...
var locales = new util_exports.Register(locales_default);
...
for (const format in import_citeproc.default.Output.Formats) {
  const original = import_citeproc.default.Output.Formats[format]["@bibliography/entry"];
  if (!original || original[proxied]) {
    continue;
  }
  import_citeproc.default.Output.Formats[format]["@bibliography/entry"] = getWrapperProxy(original);
}
...
var engines = {};
var fetchEngine = function(styleName, locale, styleXml, retrieveItem, retrieveLocale2) {
  ...
  if (engines[engineHash] instanceof import_citeproc.default.Engine) {
    ...
    engine.sys.wrapBibliographyEntry = void 0;
    engine.updateItems([]);
  } else {
...
var prepareEngine = function(data, styleName, locale, format) {
  if (!import_citeproc.default.Output.Formats[format] || !import_citeproc.default.Output.Formats[format]["@bibliography/entry"]) {
...
var engines_default = prepareEngine;

// node_modules/@citation-js/plugin-csl/lib-mjs/attr.js
var getAttributedEntry = (string, name, value) => string.replace(/^\s*<[a-z]+/i, `$& data-${name}="${value}"`);
var getPrefixedEntry = (value, id) => getAttributedEntry(value, "csl-entry-id", id);

// node_modules/@citation-js/plugin-csl/lib-mjs/bibliography.js
var getAffix = (source, affix) => typeof affix === "function" ? affix(source) : affix || "";
function bibliography(data, options = {}) {
  ...
  if (options.downgradeCsl) {
    data = util_exports.downgradeCsl(data);
  }
  const citeproc = engines_default(data, style, lang, format);
  const sortedIds = citeproc.updateItems(ids, nosort);
  if (options.append || options.prepend) {
    const items = data.reduce((items2, entry) => {
...
function citation(data, options = {}) {
  ...
  const output = citeproc.rebuildProcessorState([...before, citation2, ...after], format, []);
...
//# sourceMappingURL=@citation-js_plugin-csl.js.map
