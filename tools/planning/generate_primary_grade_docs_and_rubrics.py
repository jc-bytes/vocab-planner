...
MONTHLY_TEMPLATE = PLANS_ROOT / "Shared/Templates" / "X\u00b0 Technology - Month.docx"
...
def monthly_draft_paths(grade_folder: Path) -> list[Path]:
    root = grade_folder / "Planning" / "Drafts"
...
def generate_monthly_docx(draft: MonthlyDraft) -> Path:
    ...
    output_dir = config["folder"] / "Planning" / "Monthly" / draft.trimester_label
...
def generate_rubric_docx(summative: Summative) -> tuple[Path, Path]:
    ...
    grade_folder = GRADES[summative.grade_number]["folder"] / "Assessments" / "Rubrics"
...
def write_report(monthly_paths: list[Path], rubric_pairs: list[tuple[Path, Path]]) -> None:
    lines = [
        ...
        f"Rubric DOCX files mirrored to Shared/Generated Outputs/Rubrics 2026: {len(rubric_pairs)}",
        "",
        "## Monthly DOCX Files",
        "",
    ]
