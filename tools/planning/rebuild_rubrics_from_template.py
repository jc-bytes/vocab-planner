...
DEFAULT_TEMPLATE = DEFAULT_SOURCE / "Shared" / "Templates" / "Xth grade - XT - Week X - Rubric for Summative X.docx"
DEFAULT_OUTPUT = DEFAULT_SOURCE / "Shared/Generated Outputs/Rubrics 2026"
...
def collect_rubrics(source_root: Path, grades: set[str]) -> list[Path]:
    files: list[Path] = []
    for grade in grades:
        rubrics_dir = source_root / f"{grade} Grade Technology" / "Assessments" / "Rubrics"
        if not rubrics_dir.exists():
            continue
        files.extend(path for path in rubrics_dir.rglob("*.docx") if not path.name.startswith("~$"))
    return sorted(files)
...
def load_planning_text(source_root: Path) -> str:
    candidates = [
        ...
        source_root / "6th Grade Technology" / "Planning" / "Reviews" / "final-assessment-map.md",
        source_root / "8th Grade Technology" / "Planning" / "Reviews" / "final-assessment-map-2026.md",
    ]
    text_parts = []
...
def report_lines(
    *,
    ...
    lines = [
        "# Shared/Generated Outputs/Rubrics 2026 Report",
        "",
