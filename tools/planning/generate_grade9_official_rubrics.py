...
STALE_PRIMARY_FILES = (
    GRADE_ROOT / "Assessments" / "Rubrics" / "IT" / "9th grade - IT - Week 2 - Rubric for Summative 1.docx",
)
STALE_MIRROR_FILES = (
    GENERATED_RUBRICS_ROOT
...
ARCHIVE_ROOTS = (
    GRADE_ROOT / "Assessments" / "Rubrics" / "Archived Old Map",
...
def output_pairs() -> list[OutputPair]:
    ...
    for spec in RUBRICS:
        ...
        pairs.append(
            OutputPair(
                spec=spec,
                primary=GRADE_ROOT / "Assessments" / "Rubrics" / spec.trimester / name,
...
def write_report(pairs: list[OutputPair], archived: list[tuple[Path, Path]], dry_run: bool) -> Path:
    ...
    lines = [
        ...
        f"Template: `{PLANS_ROOT / 'Shared' / 'Templates' / 'Xth grade - XT - Week X - Rubric for Summative X.docx'}`",
        "",
        "## Generated Rubrics",
        "",
    ]
...
def main() -> int:
    ...
    if not args.skip_archive:
        for path in STALE_PRIMARY_FILES:
            destination = archive_file(path, GRADE_ROOT / "Assessments" / "Rubrics" / "Archived Old Map" / "IT")
