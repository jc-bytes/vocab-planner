...
def is_selected_kind(path: Path, kinds: set[str]) -> bool:
    ...
    if "monthly" in kinds and any("monthly planning" in part for part in normalized_parts):
        return True
    if "annual" in kinds and "annual plans" in normalized_parts:
        return True
    if "rubrics" in kinds and "rubrics" in normalized_parts:
        return True
    if "study-guides" in kinds and "study guides" in normalized_parts:
        return True
...
def source_kind(path: Path) -> str | None:
    normalized_parts = {part.lower() for part in path.parts}
    if any("monthly planning" in part for part in normalized_parts):
        return "monthly"
    if "annual plans" in normalized_parts:
        return "annual"
    if "rubrics" in normalized_parts:
        return "rubrics"
    if "study guides" in normalized_parts:
        return "study-guides"
...
def main() -> int:
    ...
    if args.backup_dir:
        ...
        backup_root = ROOT / "backups" / f"school-word-sync-{stamp}"
